...
import sepconv
import sys
from FeatureReconstructionLoss import FeatureReconstructionLoss
...
class KernelEstimation(torch.nn.Module):
    """
    The main Kernel Estimation class.
    """
...
class SepConvNet(torch.nn.Module):
    def __init__(self, kernel_size):
        ...
        # TODO: add argument for choosing loss and weights for combined loss
        self.lfe =  FeatureReconstructionLoss(1,1,1)

        # self.criterion = torch.nn.MSELoss()
        # self.criterion = torch.nn.L1Loss()
        # self.criterion = self.lfe.reconstruction_loss
        self.criterion = self.lfe.combined_loss
        lr = 5e-4
    ...
    def forward(self, frame0, frame2):
        h0 = int(frame0.shape[2])
        w0 = int(frame0.shape[3])
    ...
    # PSNR Functions
    @torch.no_grad()
    def calculate_psnr(self, frame0, frame2, frame1):
        output = self.forward(frame0, frame2)
        psnr =  -10 * torch.log10(torch.mean(((output-frame1)*(output-frame1)))).item()
        del output
        gc.collect()
    ...
    @torch.no_grad()
    def calculate_psnr_rgb(self, frame0, frame2, frame1):
        output = self.forward(frame0, frame2)
        output = output[:, :3]
        frame1 = frame1[:, :3]
        psnr =  -10 * torch.log10(torch.mean(((output-frame1)*(output-frame1)))).item()
