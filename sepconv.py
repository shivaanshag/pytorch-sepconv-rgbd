...
kernel_SeparableConvolution_updateGradVertical = '''
    extern "C" __global__ void kernel_SeparableConvolution_updateGradVertical(
        ...
        for (int intFilterX = 0; intFilterX < SIZE_1(horizontal); intFilterX++) 
        {
            ...
            VALUE_4(gradLoss, intBatch, 3, intY, intX)*                          // channel 3
            VALUE_4(input, intBatch, 3, intY + intFilterY, intX + intFilterX)* 
            VALUE_4(horizontal, intBatch, intFilterX, intY, intX);
...
class FunctionSepconv(torch.autograd.Function):
    ...
    def forward(self, input, vertical, horizontal):
        """
        Forward pass of separable convolution.
        """
        ...
        if input.is_cuda == True:
            ...
            max_threads_per_block = device.attributes.get('MaxThreadsPerBlock', 512)
            ...
            cupy_launch('kernel_Sepconv_updateOutput', cupy_kernel('kernel_Sepconv_updateOutput', {
                ...
                grid=tuple([int((n + max_threads_per_block - 1) / max_threads_per_block), 1, 1]),
                block=tuple([max_threads_per_block, 1, 1]),
    ...
    def backward(self, gradOutput):
        """
        Function performs backward pass of separable convolution
        and computes the gradients with respect to the input and filters.
        """
